...
(function(){
Ext.onReady = EventManager.onDocumentReady;
//Ext.select =  Element.select;
//Ext.select = Element.select;
Ext.encode = JSON.encode;
Ext.decode = JSON.decode;
Ext.handleError = function(e) {
    throw e;
};
Ext.reg = ComponentMgr.registerType; 
Ext.preg =  ComponentMgr.registerPlugin;
Ext.create =  ComponentMgr.create;
//Ext.layout.Accordion = Ext.layout.AccordionLayout;
//Ext.Slider = SingleSlider;
//Ext.WindowMgr = new WindowGroup();
...
Ext.apply(Ext, {
	removeNode : Ext.isIE && !Ext.isIE8 ? function(){
		var d;
		return function(n){
			if(n && n.tagName != 'BODY'){
				(Ext.enableNestedListenerRemoval) ? EventManager.purgeElement(n, true) : EventManager.removeAll(n);
...
/*(function(){
	...
	function fly(el) {
        if (!libFlyweight) {
            libFlyweight = new Element.Flyweight();
        }
        libFlyweight.dom = el;
        return libFlyweight;
    }
...
Ext.lib.Event = function() {
    ...
    var pub =  {
        ...
        getListeners : function(el, eventName) {
           EventManager.getListeners(el, eventName);
        },

        
        purgeElement : function(el, recurse, eventName) {
            EventManager.purgeElement(el, recurse, eventName);
        },
...
(function(){
    ...
    EXTLIB.AnimBase.prototype = {
        ...
        init: function(el, attributes, duration, method) {
            ...
            me.onStart = new Event(me);
            me.onTween = new Event(me);
            me.onComplete = new Event(me);
            (me._onStart = new Event(me)).addListener(onStart);
            (me._onTween = new Event(me)).addListener(onTween);
            (me._onComplete = new Event(me)).addListener(onComplete);
        }
    };
