...
var XTemplate = function(){
    ...
    while((m = s.match(re))){
       ...
       if (m3) {
           ...
           if(exp){
               fn = new Function(VALUES, PARENT, XINDEX, XCOUNT, WITHVALUES + RETURN +(Format.htmlDecode(exp))+'; }');
           }
       }
       if (m4) {
           exp = m4 && m4[1] ? m4[1] : null;
           if(exp){
               exec = new Function(VALUES, PARENT, XINDEX, XCOUNT, WITHVALUES +(Format.htmlDecode(exp))+'; }');
           }
       }
       if(name){
           switch(name){
